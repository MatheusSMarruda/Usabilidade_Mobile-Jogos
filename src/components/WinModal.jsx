...
const WinModal = ({ result, onPlayAgain, onMenu }) => {
  const { score, moves, seconds, difficulty, matchedAnimals, outcome = 'won' } = result;
  ...
  const performanceMsg = () => {
    if (isLost) return { msg: 'Sua pontuação chegou a zero. Tente novamente! 🌱', color: 'danger' };
  ...
  const titleText = isLost ? 'Game Over' : 'Parabéns!';
  const titleColor = isLost ? 'text-danger' : 'text-success';
  const headerEmoji = isLost ? '💀' : '🎉';
  const scoreBoxClass = isLost
    ? 'bg-danger bg-opacity-10 rounded-3 py-3 mb-3'
    : 'bg-success bg-opacity-10 rounded-3 py-3 mb-3';
  const scoreNumberClass = isLost
    ? 'display-5 fw-bold text-danger'
    : 'display-5 fw-bold text-success';
      ...
      <div className="win-modal card shadow-lg border-0" style={{ maxWidth: '500px', width: '95%' }}>
        <div className="card-body p-4 text-center">
          ...
          <div className="win-confetti mb-2">
            {headerEmoji.repeat(3)}
          </div>
          <h2 id="win-modal-title" className={`fw-bold ${titleColor} mb-1`}>
            {titleText}
          </h2>
          <p className={`text-${perf.color} fw-semibold mb-3`}>{perf.msg}</p>

          {/* Estrelas (oculto na derrota) */}
          {!isLost && (
          ...
          {/* Pontuação destaque */}
          <div className={`score-highlight ${scoreBoxClass}`}>
            <div className={scoreNumberClass}>{score}</div>
          ...
          <div className="d-grid gap-2">
            <button
              className={`btn ${isLost ? 'btn-danger' : 'btn-success'} btn-lg fw-bold`}
              onClick={onPlayAgain}
              aria-label={isLost ? 'Tentar novamente na mesma dificuldade' : 'Jogar novamente na mesma dificuldade'}
            >
              {isLost ? '🔁 Tentar Novamente' : '🔃 Jogar Novamente'}
            </button>
            <button
